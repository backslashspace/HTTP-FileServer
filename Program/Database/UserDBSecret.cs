    ...
    internal static partial class UserDB
    {
        ...
        private static Boolean GetSecret()
        {
            ...
            try
            {
                ...
                // todo: check file size beforehand
                fileStream.ReadExactly(Secret, 0, SECRET_SIZE);
        ...
        private static Boolean PutSecret()
        {
            if (File.Exists(Program.AssemblyPath + "\\" + SECRET_FILENAME))
            {
                Log.FastLog("Secret already present! Server attempted to create a new database, please remove the already present database secret in: " + Program.AssemblyPath + "\\" + SECRET_FILENAME +" \nNote that this file is required to use the associated database.", LogSeverity.Error, "Init");
                return false;
            }
            ...
            try
            {
                FileStream fileStream = new(Program.AssemblyPath + "\\" + SECRET_FILENAME, FileMode.Create, FileAccess.Write, FileShare.Read, SECRET_SIZE, FileOptions.SequentialScan);
