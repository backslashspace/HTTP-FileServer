...
namespace Server
{
    internal static partial class Worker
    {
        internal static void DownloadFile(Socket connection, String header, ref readonly UserDB.User invokingUser)
        {
            ...
            if (!File.Exists("\\\\?\\" + AssemblyPath + "\\files\\" + targetUsername + "\\" + filename))
            {
            ...
            FileInfo fileInfo = new("\\\\?\\" + AssemblyPath + "\\files\\" + targetUsername + "\\" + filename);
            ...
            if (fileInfo.Length > 65535)
            {
                ...
                if (BufferedSend(connection, "\\\\?\\" + AssemblyPath + "\\files\\" + targetUsername + "\\" + filename, invokingUser.LoginUsername, headerBuffer))
                {
            ...
            {
                Byte[] buffer = File.ReadAllBytes("\\\\?\\" + AssemblyPath + "\\files\\" + targetUsername + "\\" + filename);
